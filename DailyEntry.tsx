    ...
    <div className="flex items-center justify-between bg-slate-50 p-3 rounded-2xl border border-transparent hover:border-slate-200 transition-all">
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="bg-emerald-100/50 border border-emerald-200/50 rounded-xl px-3 py-1 min-w-[3.5rem] flex items-center justify-center">
          <span className="text-lg font-black text-emerald-800 header-font tabular-nums">{log.athkar.counters[field]}</span>
        </div>
        <button 
    ...
    <div className="space-y-6 pb-12 animate-in fade-in duration-500">
        ...
        <div className={`space-y-6 transition-all duration-300 ${isPerformed ? 'opacity-100' : 'opacity-40 grayscale pointer-events-none'}`}>
          <div className="flex items-center justify-between p-4 bg-emerald-50 rounded-2xl border border-emerald-100">
            <div className="flex items-center gap-3">
              <div className="p-2 rounded-xl bg-white shadow-sm"><Users className={`w-5 h-5 ${log.prayers[activePrayer].inCongregation ? 'text-emerald-600' : 'text-slate-300'}`} /></div>
              <div><h4 className="font-bold text-slate-800 text-sm header-font">صلاة الجماعة</h4><p className="text-[10px] text-slate-500 font-bold">في المسجد / جماعة أهل البيت</p></div>
            </div>
          ...
          <div className="space-y-4 px-2 pt-4">
            <div className="flex justify-between items-center"><div className="flex items-center gap-2"><Heart className="w-4 h-4 text-rose-500" /><span className="text-sm font-bold text-slate-700 header-font">مستوى الخشوع</span></div><span className="text-xs font-bold text-emerald-600 bg-emerald-50 px-2 py-1 rounded-lg">{getTranquilityLabel(log.prayers[activePrayer].tranquility)}</span></div>
      ...
      <div className="bg-white rounded-3xl p-6 shadow-sm border border-slate-100">
        ...
        {isManagingSunnahs && (
          <div className="mb-6 p-4 bg-slate-50 rounded-2xl border border-slate-200 space-y-3 animate-in slide-in-from-top duration-300">
            <div className="flex items-center justify-between mb-2"><span className="text-[10px] font-black text-slate-400 uppercase tracking-widest header-font">إضافة سنة جديدة</span><button onClick={() => setIsManagingSunnahs(false)} className="text-slate-400 hover:text-rose-500"><X className="w-4 h-4" /></button></div>
            <div className="flex gap-2">
              <input type="text" value={newSunnahName} onChange={(e) => setNewSunnahName(e.target.value)} placeholder="مثال: بر الوالدين، صلة رحم.." className="flex-1 px-4 py-2 bg-white border border-slate-200 rounded-xl text-xs font-bold header-font focus:border-emerald-500 outline-none" />
              <input type="number" value={newSunnahPoints} onChange={(e) => setNewSunnahPoints(parseInt(e.target.value) || 0)} className="w-16 px-2 py-2 bg-white border border-slate-200 rounded-xl text-xs font-bold font-mono text-center outline-none focus:border-emerald-500" />
              <button onClick={addSunnah} className="p-2 bg-emerald-600 text-white rounded-xl hover:bg-emerald-700 shadow-sm"><Plus className="w-5 h-5" /></button>
            </div>
            <div className="h-px bg-slate-200 my-4" />
            <div className="space-y-2">{(weights.customSunnahs || []).map(s => (
                <div key={s.id} className="flex items-center justify-between p-2 bg-white rounded-xl border border-slate-100"><span className="text-[10px] font-bold text-slate-600 header-font">{s.name} ({s.points}ن)</span><button onClick={() => removeSunnah(s.id)} className="p-1.5 text-rose-400 hover:bg-rose-50 rounded-lg"><Trash2 className="w-3 h-3" /></button></div>
              ))}</div>
          </div>
        )}
        <div className="grid grid-cols-1 gap-2">{(weights.customSunnahs || []).length > 0 ? (weights.customSunnahs || []).map((sunnah) => (
              <button key={sunnah.id} onClick={() => toggleCustomSunnah(sunnah.id)} className={`flex items-center justify-between p-4 rounded-2xl border transition-all ${(log.customSunnahIds || []).includes(sunnah.id) ? 'bg-emerald-50 border-emerald-300 text-emerald-800 shadow-sm' : 'bg-slate-50 border-slate-100 text-slate-500 hover:bg-white hover:border-emerald-100'}`}>
                <div className="flex items-center gap-3"><div className={`w-5 h-5 rounded-md border flex items-center justify-center transition-colors ${ (log.customSunnahIds || []).includes(sunnah.id) ? 'bg-emerald-600 border-emerald-600' : 'bg-white border-slate-300' }`}>{(log.customSunnahIds || []).includes(sunnah.id) && <Check className="w-3 h-3 text-white" />}</div><span className="text-sm font-bold header-font">{sunnah.name}</span></div>
      ...
      <div className="flex gap-4">
        <div className="flex-1 bg-white p-4 rounded-2xl shadow-sm border border-slate-100">
          ...
          <div className="flex gap-2">{[1.0, 1.05, 1.1].map(f => (
      ...
      <div className="bg-white rounded-3xl p-6 shadow-sm border border-slate-100">
        ...
        <div className="space-y-3">{counterItem('الصلاة على النبي', 'salawat', <Zap className="w-4 h-4" />)}{counterItem('الحوقلة', 'hawqalah', <Zap className="w-4 h-4" />)}{counterItem('لا إله إلا الله', 'tahlil', <Zap className="w-4 h-4" />)}{counterItem('الباقيات الصالحات', 'baqiyat', <Zap className="w-4 h-4" />)}{counterItem('الاستغفار', 'istighfar', <Zap className="w-4 h-4" />)}</div>
      </div>
      ...
      <div className="bg-white rounded-3xl p-6 shadow-sm border border-slate-100">
        ...
        <div className="space-y-4">{(['duhaDuration', 'witrDuration', 'qiyamDuration'] as const).map(field => {
          ...
          <button onClick={() => updateSection('nawafil', { fasting: !log.nawafil.fasting })} className={`w-full p-4 rounded-3xl border flex items-center justify-between mt-4 transition-all ${log.nawafil.fasting ? 'bg-orange-500 border-orange-500 text-white shadow-lg' : 'bg-slate-50 border-slate-100 text-slate-500'}`}><div className="flex items-center gap-2"><Sun className="w-5 h-5" /><span className="font-bold text-sm header-font">صيام (نفل / قضاء)</span></div>{log.nawafil.fasting ? <CheckCircle2 className="w-5 h-5" /> : <div className="w-5 h-5 border-2 border-slate-300 rounded-full" />}</button>
        </div>
      </div>
