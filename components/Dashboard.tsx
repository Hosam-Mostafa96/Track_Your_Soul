...
import { 
  Flame, Target, Sparkles, Edit2, Check, BrainCircuit, X, 
  Activity, History, Award, Sun, Moon, Heart, CloudMoon, 
  Home, Key, Coins, CheckCircle2, Lock
} from 'lucide-react';
...
const Dashboard: React.FC<DashboardProps> = ({ log, logs, weights, onDateChange, targetScore, onTargetChange, onOpenSettings }) => {
  ...
  const getAiAdvice = async () => {
    // Correctly accessing API_KEY from process.env and initializing SDK
    const apiKey = (process.env as any).API_KEY;
    if (!apiKey) {
      setAiAdvice("النية الصالحة هي روح العمل، واصل مسيرك بارك الله فيك ونفع بك. (مفتاح API غير متوفر)");
      return;
    }
    setIsAiLoading(true);
    try {
      ...
      const response = await ai.models.generateContent({
        ...
        contents: `أنا مستخدم لتطبيق إدارة عبادات وأوراد. مجموع نقاطي اليوم هو ${currentTotalScore} من هدف يومي قدره ${targetScore}. مستوى أدائي حالياً مقارنة بالأسبوع الماضي هو ${momentumInfo.percent}%. أعطني نصيحة روحية قصيرة جداً بأسلوب مشجع.`,
      });
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <button onClick={getAiAdvice} disabled={isAiLoading} className="w-full bg-gradient-to-r from-emerald-600 to-teal-700 p-4 rounded-3xl shadow-lg flex items-center justify-between group active:scale-95 transition-all text-white">
        <div className="flex items-center gap-3">
          <div className="p-2 bg-white/20 rounded-xl"><BrainCircuit className={`w-5 h-5 ${isAiLoading ? 'animate-pulse' : ''}`} /></div>
          <div className="text-right"><h4 className="text-sm font-bold header-font">المستشار الروحي (AI)</h4><p className="text-[10px] opacity-80 header-font">اضغط للحصول على نصيحة لأورادك</p></div>
        </div>
        <Sparkles className={`w-5 h-5 text-yellow-300 ${isAiLoading ? 'animate-spin' : 'animate-bounce'}`} />
      </button>

      {aiAdvice && (
        <div className="bg-emerald-50 border border-emerald-100 p-4 rounded-2xl relative animate-in slide-in-from-top duration-300">
      ...
      <div className="bg-white rounded-3xl p-6 shadow-sm border border-slate-100">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {badges.map(badge => (
            <div key={badge.id} className={`flex flex-col items-center p-3 rounded-2xl border transition-all duration-500 ${badge.active ? `bg-gradient-to-br ${badge.color} text-white border-transparent` : 'bg-slate-50 text-slate-400 border-slate-100'}`}>
              <div className={`p-2 rounded-xl mb-2 ${badge.active ? 'bg-white/20' : 'bg-slate-200'}`}>{badge.active ? badge.icon : <Lock className="w-4 h-4" />}</div>
              <span className="text-[9px] font-black header-font text-center leading-tight">{badge.title}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="bg-white rounded-3xl p-6 shadow-sm border border-slate-100">
        <div className="flex justify-between items-center mb-4">
          ...
          <button onClick={() => setIsEditingTarget(true)} className="text-xs font-bold text-slate-500 bg-slate-50 px-3 py-1 rounded-xl">{targetScore.toLocaleString()}</button>
        </div>
        <div className="w-full bg-slate-100 h-3 rounded-full overflow-hidden"><div className="bg-emerald-500 h-full transition-all duration-1000" style={{ width: `${progressPercent}%` }} /></div>
      </div>

      <div className="h-48 w-full bg-white p-6 rounded-3xl shadow-sm border border-slate-100">
