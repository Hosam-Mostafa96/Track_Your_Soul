...
const WorshipTimer: React.FC<WorshipTimerProps> = ({ 
  ...
  useEffect(() => { localStorage.setItem('mizan_anon_id', anonId.current); }, []);
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 pb-20">
      <div className="bg-white rounded-[2.5rem] p-10 shadow-sm border border-slate-100 flex flex-col items-center relative overflow-hidden">
        ...
        {isRunning && (
          <div className="absolute top-6 right-6 flex items-center gap-2 bg-rose-50 px-3 py-1 rounded-full animate-in fade-in zoom-in border border-rose-100">
        ...
        <div className="text-8xl font-black font-mono text-emerald-900 mb-4 mt-8 tabular-nums tracking-tighter">
          {formatTime(seconds)}
        </div>

        <p className="text-[11px] font-bold text-slate-400 header-font mb-10 uppercase tracking-widest">
          {isRunning ? 'العداد يعمل الآن في الخلفية' : 'المؤقت متوقف'}
        </p>
        ...
        <div className="grid grid-cols-2 gap-2 w-full mb-10">
          {activities.map(a => (
            <button
              key={a.id}
              onClick={() => !isRunning && onActivityChange(a.id)}
              className={`flex items-center gap-2 px-4 py-3 rounded-2xl border transition-all text-xs font-bold header-font ${selectedActivity === a.id ? 'bg-emerald-600 border-emerald-600 text-white shadow-lg' : 'bg-slate-50 border-transparent text-slate-400'}`}
            >
              {a.icon}
